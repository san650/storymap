# story-color-coding Specification

## Purpose

Allow story cards to carry an optional user-chosen color override (green default, sky, or lilac), selectable via an inline swatch picker while editing. The override affects display only while the card is in the story band, flows through the command pipeline for undo/redo and persistence, and survives export/import.
